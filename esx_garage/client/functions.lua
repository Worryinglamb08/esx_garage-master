    ...
    ESX.TriggerServerCallback("garage:fetchPlayerVehicles", function(fetchedVehicles)
        ...
        for key, vehicleData in ipairs(fetchedVehicles) do
            ...
            table.insert(menuElements, {
                ["label"] = "Nom du véhicule: " .. GetLabelText(GetDisplayNameFromVehicleModel(vehicleProps["model"])) .. " avec la plaque: " .. vehicleData["plate"],
                ["vehicle"] = vehicleData
            })
        end
        ...
        if #menuElements == 0 then
            table.insert(menuElements, {
                ["label"] = "Vous n'avez pas garé de véhicule ici."
            })
...
OpenVehicleMenu = function()
    ESX.TriggerServerCallback("garage:fetchPlayerVehicles", function(fetchedVehicles)
            ...
            for _, vehicle in ipairs(gameVehicles) do
                if DoesEntityExist(vehicle) then
                    ...
                    if Config.Trim(GetVehicleNumberPlateText(vehicle)) == Config.Trim(vehicleProps["plate"]) then
                        table.insert(menuElements, {
                            ["label"] = GetLabelText(GetDisplayNameFromVehicleModel(vehicleProps["model"])) .. " avec la plaque - " .. vehicleData["plate"] .. " - " .. dstCheck .. " mètres plus loin.",
        ...
        if #menuElements == 0 then
            table.insert(menuElements, {
                ["label"] = "Vous n'avez pas de véhicule."
            })
        end
...
ChooseVehicleAction = function(vehicleEntity, callback)
    ...
    local menuElements = {
        {
            ["label"] = (GetVehicleDoorLockStatus(vehicleEntity) == 1 and "Lock" or "Unlock") .. " véhicule.",
            ["action"] = "change_lock_state"
        },
        {
            ["label"] = "Turn " .. (GetIsVehicleEngineRunning(vehicleEntity) and "off" or "on") .. " le moteur.",
...
VehicleAction = function(vehicleEntity, action)
    ...
    if action == "change_lock_state" then
        if dstCheck >= Config.RangeCheck then return ESX.ShowNotification("Vous êtes trop loin de votre véhicule pour contrôler.") end
    ...
    elseif action == "change_gps_state" then
        if DoesBlipExist(cachedData["blips"][vehicleEntity]) then
            ...
            AddTextComponentString("Véhicule personel - " .. GetVehicleNumberPlateText(vehicleEntity))
...
SpawnVehicle = function(vehicleProps)
	...
	for i = 1, #gameVehicles do
        ...
        if DoesEntityExist(vehicle) then
			if Config.Trim(GetVehicleNumberPlateText(vehicle)) == Config.Trim(vehicleProps["plate"]) then
				ESX.ShowNotification("Ce véhicule est dans la rue, vous ne pouvez pas sortir 2 des mêmes véhicules.")
	...
	ESX.Game.SpawnVehicle(vehicleProps["model"], spawnpoint["position"], spawnpoint["heading"], function(yourVehicle)
        ...
        ESX.ShowNotification("~b~[Information]:~s~ Vous avez réussi à sortir votre véhicule.")
	...
	if DoesEntityExist(vehicle) then
		...
		ESX.TriggerServerCallback("garage:validateVehicle", function(valid)
			if valid then
				...
				ESX.ShowNotification("~b~[Information]:~s~ Vous avez garé votre véhicule avec succès.")
			else
				ESX.ShowNotification("~b~[Information]:~s~ Êtes-vous sûr que la véhicule vous appartient?")
			end
...
WaitForModel = function(model)
    ...
    if not IsModelValid(model) then
        return ESX.ShowNotification("Ce modèle n'existe pas dans le jeu.")
    end
	...
	while not HasModelLoaded(model) do
		...
		DrawScreenText("Nom de la véhicule" .. GetLabelText(GetDisplayNameFromVehicleModel(model)) .. "...", 255, 255, 255, 150)
	end
end